...
class KategoriBukuController extends Controller
{
    ...
    {
        if (
            KategoriBuku::where('no_urut', $request->no_urut)->first() != null
        ) {
    ...
    {
        $query = KategoriBuku::select('id', 'no_urut', 'nama_kategori')
            ->orderBy('no_urut', "ASC")
            ->get();
    ...
    public function get(Request $request)
    {
        return response()->json(
            KategoriBuku::where('id', $request->id)->first(),
            200
        );
    }
    ...
    {
        $existing = KategoriBuku::where('no_urut', $request->no_urut)
            ->where('id', '!=', $request->id)
            ->first();
        ...
        KategoriBuku::where('id', $request->id)->update([
            'no_urut' => $request->no_urut,
            'nama_kategori' => $request->nama_kategori,
        ]);
