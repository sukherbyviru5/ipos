@extends('guest')
@section('title', 'Buku')
@section('meta_description', 'Pustaka buku')
...
@section('content')
    ...
    <div class="container">
        <div class="section-heading d-flex align-items-center justify-content-between dir-rtl py-3">
            <h6>Daftar Buku</h6>
        </div>
        <div class="row g-2">
                    ...
                    <div class="card product-card">
                        <div class="card-body">
                            <a class="product-thumbnail d-block" href="{{ route('buku.show', $item->singkatan_buku) }}">
                                <img class="mb-2" src="{{ asset($item->cover_buku ?? 'assets/img/default-book.webp') }}" alt="">
                            </a>
                            <a class="product-title" style="font-size: 13px;" href="{{ route('buku.show', $item->singkatan_buku) }}">{{ $item->judul_buku }}</a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="discount-coupon-card p-4 p-lg-5 dir-rtl">
                    <div class="d-flex align-items-center">
                        <div class="discountIcon"><img class="w-100" src="{{ asset('assets/img/404.png') }}" alt=""></div>
                        <div class="text-content">
                            <h5 class="text-white mb-2">Uppss..</h5>
                            <p class="text-white mb-0">Tidak ada buku ditemukan.</p>
                        </div>
                    </div>
                </div>
            @endforelse
